...
This vignette demonstrates the package’s ENVI and calibration interfaces
using synthetic data. Device support tests use fixtures and SDK mocks;
real camera sessions remain an external validation requirement. Read
wavelength, FWHM, processing mode and acquisition settings from each
recording. Do not infer them from a model name or this demonstration’s
defaults.
...
# Demonstrate the calibration arithmetic (these references are synthetic)
